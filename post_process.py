...
if __name__ == '__main__':
    ...
    rawMeanCostListByNumOfTasks = [[]] * 11 # 0 is not used.
    for i in xrange(1,9):
        summaryFilePath = logFolderPath + "/" + SUB_LOG_FOLDER_NAME.format(i) + "/" + SUB_LOG_FOLDER_NAME
        with open(summaryFilePath, "r") as lines:
            for line in lines:
                numOfTasks = int(line.split(":")[0])
    ...
    for i in xrange(1, 11):
        meanMeanCostList[i] = sum(rawMeanCostListByNumOfTasks[i])/rawMeanCostListByNumOfTasks[i]
